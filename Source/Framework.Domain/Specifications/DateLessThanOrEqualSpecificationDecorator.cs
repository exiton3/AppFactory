...
{
    public class DateLessThanOrEqualSpecificationDecorator<T> : SpecificationBase<T> where T : class
    {
        private readonly string _property;
        private readonly DateTime? _value;
        private readonly ISpecification<T> _specification;

        public DateLessThanOrEqualSpecificationDecorator(ISpecification<T> specification, object value, string propertyName)
        {
        ...
        public override Expression<Func<T, bool>> GetExpression()
        {
            ...
            if (propType == typeof(DateTime?))
            {
                ...
                var condition = Expression.LessThanOrEqual(Expression.Constant(_value),
                    Expression.Convert(memberExp, typeof(DateTime)));
                gtExpression = Expression.AndAlso(nullCheck,condition);
            }
            if (propType == typeof(DateTime))
            {
                gtExpression = Expression.LessThanOrEqual(Expression.Constant(_value ?? (DateTime)SqlDateTime.MinValue), memberExp);
            }
